...
| Peer validation does not detect sparse filers | Foreign banks and similar names (e.g. RY, TD in JPM's peer set) | Peer validation drops names with no US-GAAP XBRL data, but does not detect sparse filers. Foreign banks and similar names with thin us-gaap sections still enter peer medians. A concept-count threshold was considered and rejected as arbitrary. |
